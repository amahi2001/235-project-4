...
{
   this->invertRest(this->getHeadPtr()); // calling the inverRest() function
}

template <class T>
void LinkedList<T>::rotate(int k)
{
   bool flag = k == 0 || item_count_ < 1 || k < 0; //conditions that will not work
   int thisSize = this->getLength(); //variable that's the length of the linked list
   switch (flag)
   {
   case true: //if the error conditions are true
      return; //exit out of the function
      break;
   default: //else
      Node<T> *newHead = getNodeAt(thisSize - 1);  //create a temp head ptr that points to the second to last node
      newHead->setNext(head_ptr_);                 //point it to the head ptr
      Node<T> *lastPtr = getNodeAt(thisSize - 2);  //create a last ptr and point it to the 3rd to last 
      lastPtr->setNext(nullptr);                   //create a new poiunter and let it point to null
      head_ptr_ = newHead;                         //point the lastptr to the new head   
      rotate((k - 1) % thisSize);                  //recursive call to rotate with new parameters
   }
}
...
{
   while (current_first_ptr == nullptr) 
   {
      return;
   }

   bool flag = current_first_ptr->getNext() == nullptr;

   switch (flag) //if the curr pointer is pointing to the null (aka the last ptr on the list)
   {
   case true:
      head_ptr_ = current_first_ptr; //set that node to be a headptr
      return;                        //returning void
      break;
   }
   invertRest(current_first_ptr->getNext()); //recuursively calling while moving to the next pointer
   current_first_ptr->getNext()->setNext(current_first_ptr); //calling the most recent stack and repointing the pointer to nmext for the stack value of curr_first_pointer
   current_first_ptr->setNext(nullptr);                     //deleiting the old pointer
}
